...
class data_collect(object):
    ...
    def collectDATA(self,in_code,start_dt,end_dt):
        ...
        cursor = db.cursor()
        stocktable = 'stock' + in_code[:6]
        sql_done_set = "SELECT * FROM %s where stock_code = '%s' and state_dt >= '%s' and state_dt <= '%s' order by state_dt asc" % (stocktable, in_code, start_dt, end_dt)
        print(sql_done_set)
